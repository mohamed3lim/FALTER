...
#define S_FUNCTION_NAME  minSensorResults_old
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetOutputPortWidth(S, 0, SENSOR_WIDTH);
...
void mdlSetDefaultPortDimensionInfo(SimStruct *S) {
        if(ssGetInputPortWidth(S, 0) == DYNAMICALLY_SIZED ) {
            ssSetInputPortMatrixDimensions(S, 0, 1, SENSOR_WIDTH);
        }
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int i, j, count = uWidth/SENSOR_WIDTH;
    
    for( i = 0; i < SENSOR_WIDTH; i++ ) {
        real_T min = 1000;
        for( j = 0; j < count; j++) {
            real_T v = u[i + j*SENSOR_WIDTH];
            if( v < min && v != 0 ) min = v;
        }
        y[i] = min;
    }
    
    qsort(y, yWidth, sizeof(real_T), compare_real_T);
    
    real_T current, last = 0.0;    
    for( i = 0; i < SENSOR_WIDTH; i++ ) {
        current = y[i];
        if( current - last < SENSOR_MIN_DIFF) {
            y[i] = 0;
